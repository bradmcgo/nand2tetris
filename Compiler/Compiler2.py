...
class SymbolTable:
    ...
    def define(self, name, type, kind):
        ...
        if kind == "STATIC":
            self.add_row_class(name, type, kind, staticIndex)
            staticIndex += 1

        if kind == "FIELD":
            self.add_row_class(name, type, kind, fieldIndex)
            fieldIndex += 1

        if kind == "VAR":
            self.add_row_class(name, type, kind, varIndex)
            varIndex += 1

        if kind == "ARG":
            self.add_row_class(name, type, kind, argIndex)
            argIndex += 1
...
class CompilationEngine:
    ...
    def _typeOfChar(self):
        ...
        if self.jackTokenizer.tokenType() == "IDENTIFIER":
            ...
            if kind in ["ARG", "VAR"]:
                return self.outputFile.write(f"<identifier> {self.jackTokenizer.identifier()} </identifier><type> {self.subSymbolTable.typeOf(self.current_name)} </type><kind> {self.subSymbolTable.kindOf(self.current_name)} </kind><index> {self.subSymbolTable.indexOf(self.current_name)} </index>\n")
            elif kind in ["STATIC", "FIELD", "NONE"]:
                return self.outputFile.write(f"<identifier> {self.jackTokenizer.identifier()} </identifier><type> {self.classSymbolTable.typeOf(self.current_name)} </type><kind> {self.classSymbolTable.kindOf(self.current_name)} </kind><index> {self.classSymbolTable.indexOf(self.current_name)} </index>\n")
    ...
    def _checkStar(self):
        while True:
            if self.jackTokenizer.symbol() == ",":
                ...
                kind = self.subSymbolTable.kindOf(self.jackTokenizer.identifier())
                if kind in ["ARG", "VAR"]:
                    self.outputFile.write(f"<identifier> {self.jackTokenizer.identifier()} </identifier><type> {self.subSymbolTable.typeOf(self.current_name)} </type><kind> {self.subSymbolTable.kindOf(self.current_name)} </kind><index> {self.subSymbolTable.indexOf(self.current_name)} </index>\n")
                elif kind in ["STATIC", "FIELD", "NONE"]:
                    self.outputFile.write(f"<identifier> {self.jackTokenizer.identifier()} </identifier><type> {self.classSymbolTable.typeOf(self.current_name)} </type><kind> {self.classSymbolTable.kindOf(self.current_name)} </kind><index> {self.classSymbolTable.indexOf(self.current_name)} </index>\n")
